...
const writings = [
  {
    ...
    tags: ['碎片', '个人', '夜晚'],
    preview:
      '我想要的不是一个普通主页，而是一个可以让我一打开就想开始创作的地方。',
  },
  {
    id: 2,
    title: '给未来作品集的草稿',
  ...
  },
  {
    ...
    tags: ['视觉', '灵感采样'],
    preview:
      '我最近喜欢偏雾感的绿色和有一点颗粒感的奶油白，它们很像慢慢展开的画布。',
  },
];
...
function StatCard({
  ...
  return (
    <Card className="rounded-3xl border-zinc-200/70 bg-white/70 shadow-sm backdrop-blur">
    ...
    <button
      ...
      className={
        active
          ...
          : 'rounded-xl px-4 py-2 text-sm text-zinc-600'
      }
    >
      {children}
    </button>
...
export default function Page() {
  ...
  const [draft, setDraft] = useState(
    '今天想写点什么？可以是突然冒出来的一句话，也可以是某个作品未来会长成的样子。'
  );
    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(226,232,240,0.9),_rgba(250,250,249,1)_35%,_rgba(245,245,244,1)_70%)] text-zinc-900">
      <div className="mx-auto grid min-h-screen max-w-7xl grid-cols-1 gap-6 px-4 py-6 md:px-6 lg:grid-cols-[250px_minmax(0,1fr)_330px]">
        <motion.aside
          ...
          className="rounded-[28px] border border-zinc-200/70 bg-white/75 p-4 shadow-sm backdrop-blur"
        >
          ...
          <div className="mt-6 rounded-3xl bg-gradient-to-br from-zinc-900 to-zinc-700 p-4 text-white">
            ...
            <p className="mt-2 text-sm text-white/80">
              帮你命名、总结、推荐标签，也可以在你卡住的时候给你一个继续写下去的提示。
            </p>
            <Button className="mt-4 w-full rounded-2xl bg-white text-zinc-900 hover:bg-zinc-100">
              打开创作助手
            </Button>
          </div>
        </motion.aside>
        ...
        <main className="space-y-6">
          <motion.section
            ...
            className="overflow-hidden rounded-[32px] border border-zinc-200/70 bg-white/70 p-6 shadow-sm backdrop-blur"
          >
            <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
              <div className="max-w-2xl">
                <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-zinc-200 bg-white/80 px-3 py-1 text-xs text-zinc-500">
                  <Heart className="h-3.5 w-3.5" /> a beautiful workspace for
                  writing, art, and experiments
                </div>
                ...
                <p className="mt-4 max-w-xl text-sm leading-6 text-zinc-600 md:text-base">
                  这里不只是主页，而是你的工作台：随时写随笔、上传画作、收纳模型实验，并自动整理成清晰的个人档案。
                </p>
              </div>
          ...
          {tab === 'writing' && (
            <div className="grid grid-cols-1 gap-4 xl:grid-cols-[1.2fr_0.8fr]">
              <Card className="rounded-[28px] border-zinc-200/70 bg-white/80 shadow-sm">
                <CardHeader>
                  <CardTitle className="flex items-center gap-2 text-xl">
                    <PenSquare className="h-5 w-5" /> 今日创作
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <Input
                    placeholder="给这篇内容起个名字..."
              ...
              <Card className="rounded-[28px] border-zinc-200/70 bg-white/70 shadow-sm">
                <CardHeader>
                  <CardTitle className="flex items-center gap-2 text-xl">
                    <BookOpen className="h-5 w-5" /> 最近文字
                  </CardTitle>
                </CardHeader>
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          ...
                          <div className="mt-1 flex flex-wrap gap-2 text-xs text-zinc-500">
                            ...
                            <span className="inline-flex items-center gap-1">
                              <Clock3 className="h-3.5 w-3.5" /> {item.updated}
                            </span>
                          </div>
                        </div>
            ...
            <>
              <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                {gallery.map((work, i) => (
                  ...
                  >
                    <Card className="overflow-hidden rounded-[28px] border-zinc-200/70 bg-white/80 shadow-sm">
              ...
              <Card className="rounded-[28px] border-dashed border-zinc-300 bg-white/70 shadow-sm">
                <CardContent className="flex flex-col items-center justify-center gap-3 p-10 text-center">
                  <Upload className="h-8 w-8 text-zinc-400" />
                  <div className="text-lg font-medium">上传新的画作或系列</div>
                  <p className="max-w-md text-sm text-zinc-500">
                    上传后可自动提取标题、媒介、系列、颜色风格，也可以保留你自己手动定义的命名方式。
                  </p>
                  <Button className="rounded-2xl">上传作品</Button>
                </CardContent>
              </Card>
            </>
          )}
          ...
          {tab === 'models' && (
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-[1fr_320px]">
              ...
              <Card className="rounded-[28px] border-zinc-200/70 bg-gradient-to-br from-zinc-900 to-zinc-700 text-white shadow-sm">
                <CardHeader>
                  <CardTitle className="flex items-center gap-2 text-xl">
                    <Brain className="h-5 w-5" /> AI Studio Helper
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-4 text-sm text-white/85">
                  ...
                  <Button className="w-full rounded-2xl bg-white text-zinc-900 hover:bg-zinc-100">
                    试试陪伴模式
                  </Button>
                </CardContent>
              </Card>
        ...
        >
          <Card className="rounded-[28px] border-zinc-200/70 bg-white/80 shadow-sm">
            <CardHeader>
              <CardTitle className="text-xl">自动归类逻辑</CardTitle>
            </CardHeader>
          ...
          <Card className="rounded-[28px] border-zinc-200/70 bg-white/80 shadow-sm">
            <CardHeader>
              <CardTitle className="text-xl">推荐网站结构</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3 text-sm text-zinc-600">
